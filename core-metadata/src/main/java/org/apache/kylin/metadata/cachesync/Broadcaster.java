 ...
 *
 * The origin server announce the event via Rest API to all Kylin servers including itself. On
 * target server, listeners are registered to process events. As part of processing, a listener can
 * re-notify a new event to other local listeners.
 *
 * A typical project schema change event: - model is update on origin server, a "model" update event
 * is announced - on all servers, model listener is invoked, reload the model, and notify a
 * "project_schema" update event - all listeners respond to the "project_schema" update -- reload
 * cube desc, clear project L2 cache, clear calcite data source etc
 */
public class Broadcaster {
  ...
  private Broadcaster(final KylinConfig config) {
    ...
    Executors.newSingleThreadExecutor(new DaemonThreadFactory()).execute(new Runnable() {
      ...
      public void run() {
        ...
        final ExecutorService wipingCachePool = Executors
            .newFixedThreadPool(restClients.size(), new DaemonThreadFactory());
        while (true) {
          final BroadcastEvent broadcastEvent;

          try {
            broadcastEvent = broadcastEvents.takeFirst();
            logger.info("Announcing new broadcast event: " + broadcastEvent);
          } catch (Exception e) {
            logger.error("error taking event from queue ", e);
            continue;
          }
  ...
  public void notifyListener(String entity, Event event, String cacheKey) throws IOException {
    synchronized (CACHE) {
      ...
      logger.debug(
          "Broadcasting metadata change: entity=" + entity + ", event=" + event + ", cacheKey="
              + cacheKey + ", listeners=" + list);
      ...
      switch (entity) {
        ...
        case SYNC_PRJ_DATA:
          ProjectManager.getInstance(config)
              .clearL2Cache(); // cube's first becoming ready leads to schema change too
          for (Listener l : list) {
      ...
      logger.debug(
          "Done broadcasting metadata change: entity=" + entity + ", event=" + event + ", cacheKey="
              + cacheKey);
    }
  }
  ...
  public static class BroadcastEvent {
    ...
    public String toString() {
      return Objects.toStringHelper(this).add("entity", entity).add("event", event)
          .add("cacheKey", cacheKey).toString();
    }
